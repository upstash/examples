...
async function listTodos() {
  ...
  for (const todo of todos.result) {
    grid.appendChild(createCard("a", "card", todo)).addEventListener("click", (event) => removeTodo(event));
  }
}
...
window.onload = () => {
  document.querySelector(".cardForm").addEventListener("submit", (event) => addTodo(event));
  listTodos();
};
